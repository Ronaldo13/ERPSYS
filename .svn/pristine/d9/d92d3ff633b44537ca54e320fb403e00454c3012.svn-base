	...
	<div id="" style="">
		<form class="form-inline">
			<button type="button" class="btn btn-success"
				onclick="show('增加新账单','<%=basePath%>insertAccountsPayable',400,580)">
			...
			<div class="form-group">
				<input style="width: 8em;" type="text" class="form-control"
					id="searchAccountsPayableName" name="searchAccountsPayableName" placeholder="账务名称" value="${searchAccountsPayableName }">
			</div>
			<div class="form-group">
			...
			<table class="table table-hover" style="text-align: center;">
				...
				<tbody>
					<c:forEach items="${erpsysAccountsPayableList }"
						var="accountsPayables">
						<tr id="accountsPayable${accountsPayables.accountsPayableId }">
							<td><input type="checkbox" name="checks[]"
								id="${accountsPayables.accountsPayableId }"
								value="${accountsPayables.accountsPayableId }" /></td>
							<td>${accountsPayables.accountsPayableId }</td>
							<td>${accountsPayables.accountsPayableName }</td>
							<td>${accountsPayables.accountsPayableMoney }</td>
							<td>${accountsPayables.accountsReceiver }</td>
							<td>${accountsPayables.accountsRecordTimeString }</td>
							<td><c:choose>
									<c:when test="${accountsPayables.alreadyPaid==1 }">
							...
							<td>${accountsPayables.payTimeString }</td>
							<td>
								<div class="btn-group" role="group">
									<button type="button" class="btn btn-default btn-sm"
										onclick="show('应出账务单管理','<%=basePath%>editAccountsPayable?accountsPayableId=${accountsPayables.accountsPayableId }','400','650')">
										<span class="glyphicon glyphicon-pencil"></span> 编辑
									</button>
									<button type="button" class="btn btn-danger btn-sm"
										onclick="del(${accountsPayables.accountsPayableId })">
...
<script>
	...
	function doSearchName(){//简单判断查询money的值是否符合要求，并拼接域名发往后台，不用ajax是为了刷新页面的时候，继续停留在当前页
			...
			var searchAccountsPayableName = $("#searchAccountsPayableName").val();
			window.location.href="<%=basePath%>financialManagementAccountsPayable?searchName="+searchName+"&searchAccountsPayableName="+searchAccountsPayableName;
	}
	//弹出窗口调用该方法来改变表格中的值,从而不用刷新页面
	function changeTable(accountsPayableId,accountsPayableName,accountsPayableMoney,accountsReceiver,accountsRecordTimeString,alreadyPaid,payTimeString){
		//修改当前行应出账务名称
		$("#accountsPayable"+accountsPayableId+"").find("td").eq(2).text(accountsPayableName)
		//修改当前行应出账务金额
		$("#accountsPayable"+accountsPayableId+"").find("td").eq(3).text(accountsPayableMoney)
		//修改当前行应收款方
		$("#accountsPayable"+accountsPayableId+"").find("td").eq(4).text(accountsReceiver)
		//修改当前行账务记录时间
		$("#accountsPayable"+accountsPayableId+"").find("td").eq(5).text(accountsRecordTimeString)
		//修改当前行是否已出账
		//修改并赋予颜色
		if(alreadyPaid=="已出账"){
			$("#accountsPayable"+accountsPayableId+"").find("td").eq(6).html('<span style="color: green;">'+alreadyPaid+'</span>')
		}else{
			$("#accountsPayable"+accountsPayableId+"").find("td").eq(6).html('<span style="color: red;">'+alreadyPaid+'</span>')
		}
		//修改当前行出账时间
		$("#accountsPayable"+accountsPayableId+"").find("td").eq(7).text(payTimeString)
	...
	function del(id){//单个删除，不多赘述
		layer.confirm('确定要删除编号为'+id+'的应出账务单吗？', {icon: 3, title:'提示'}, function(index){
			$.ajax({
				url : "deleteAccountsPayableSubmit",
				type : "post",
				dataType : "text",
	...
	function flushPage(){
		...
		if((searchName == null || searchName == "" )&&((searchAccountsPayableName == null || searchAccountsPayableName == "" ))){//拼接url
	...
	function PLDelete(){
		...
		if(ids.length!=0){//非空则把该数组ajax发送到后台进行批量删除
			layer.confirm('确定要删除选中的应出账务单吗？', {icon: 3, title:'提示'}, function(index){
				$.ajax({
					url : "PLdeleteAccountsPayableSubmit",
					type : "post",
					dataType : "text",
					contentType : "application/x-www-form-urlencoded;charset=utf-8",
					traditional:true,//ajax传入数组，必须写
					data : {
						"accountsPayableIds" : ids
					},
					success : function(result) {
						if (result == "SUCCESS") {
							parent.showMessage('账单删除成功！')

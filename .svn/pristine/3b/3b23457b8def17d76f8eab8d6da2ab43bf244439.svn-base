...
public class FinancialManagementAccountsPayableServiceImpl implements FinancialManagementAccountsPayableService {
	...
	// 应收账务账单Mapper
	@Resource
	private ErpsysAccountsPayableMapper eapm;
	...
	public HttpServletRequest initFinancialManagementPage(HttpServletRequest request) {
		...
		try {
			...
			List<ErpsysAccountsPayable> erpsysAccountsPayableList = eapm.selectAccountsPayableByReceiverAndEnd(map);// 按需查询
			totalRecords = eapm.selectAccountsPayableByReceiverAndEndCount(map);// 查询按需筛选的总条目数
			...
			if ("yes".equals(pageEnd)) {
				...
				map.put("pageNo", pageNo);
				erpsysAccountsPayableList = eapm.selectAccountsPayableByReceiverAndEnd(map);
			}
	...
	public HttpServletRequest initEditAccountsPayablePage(HttpServletRequest request) {
		...
		if (accountsPayableIdString != null) {
			try {
				...
				ErpsysAccountsPayable accountsPayable = eapm.selectByPrimaryKey(accountsPayableId);
	...
	public int updateAccountsPayable(ErpsysAccountsPayable accountsPayable) {
		...
		return eapm.updateByPrimaryKeySelective(accountsPayable);
	}
	...
	public int insertAccountsPayable(ErpsysAccountsPayable accountsPayable) {
		...
		return eapm.insertSelective(accountsPayable);
	}
	...
	public int deleteAccountsPayable(int accountsPayableId) {
		return eapm.deleteByPrimaryKey(accountsPayableId);
	}
	...
	public int PLdeleteAccountsPayable(HttpServletRequest request) {
		...
		try {
			for (String string : accountsPayableIds) {
				...
				int j = eapm.deleteByPrimaryKey(accountsPayableId);
				i = i + j;
			}
		} catch (Exception e) {
			return 0;

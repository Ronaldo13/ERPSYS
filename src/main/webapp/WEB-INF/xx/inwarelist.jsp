...
<body>
	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach items="${inWareHouseModels}" var="inWareHouseModels">
			<tr id="warelist${inWareHouseModels.inWarehouseLogId}">
				<td><input class="ckc" type="checkbox"
					id="${inWareHouseModels.inWarehouseLogId}" name="check"
					value="${inWareHouseModels.inWarehouseLogId }"></td>
				<td>${inWareHouseModels.inWarehouseLogId }</td>
				<td>${inWareHouseModels.productionWarehouseId }</td>
				<td>${inWareHouseModels.inWarehouseTime }</td>
				<td>${inWareHouseModels.inWarehouseReason }</td>
				<td>${inWareHouseModels.inWarehouseNumber }</td>
	...
	$(function() {
		...
		kkpager.generPageHtml({
			...
			click : function(n) {
				// do something
				//手动选中按钮
				window.location.href = "ware/getAllInware?pageNo=" + n;
				this.selectPage(n);
				return false;
			}
		});

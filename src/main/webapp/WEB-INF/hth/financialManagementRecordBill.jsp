	...
	<div id="" style="">
		<form class="form-inline">
			<button type="button" class="btn btn-success"
				onclick="show('增加新账单','<%=basePath%>insertRecordBill',400,580)">
	...
	function searchMoney(){//简单判断查询money的值是否符合要求，并拼接域名发往后台，不用ajax是为了刷新页面的时候，继续停留在当前页
		...
		var endMoney = $("#endMoney").val();
			...
			if(startMoney==null || startMoney==""){
				window.location.href="<%=basePath%>financialManagementRecordBill?endMoney="+endMoney;
			}
			if(endMoney==null  || endMoney==""){
				window.location.href="<%=basePath%>financialManagementRecordBill?startMoney="+startMoney;
			}else{
				if(startMoney>endMoney){
					alert("起始值不能大于结束值！")
				}
				window.location.href="<%=basePath%>financialManagementRecordBill?startMoney="+startMoney+"&endMoney="+endMoney;
			}
	}
